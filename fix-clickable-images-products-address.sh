...
echo "== Fix clickable testimony images, product preview modal, and address =="

STAMP="$(date +%Y%m%d-%H%M%S)"
mkdir -p "backup-before-clickable-products-address-$STAMP"
cp -R frontend "backup-before-clickable-products-address-$STAMP/frontend"

cat > frontend/js/clickable-media-product-modal.js <<'JS'
(function(){
  function ready(fn){
...
# Add clickable modal JS to all main frontend pages
for f in frontend.glob("*.html"):
    text = f.read_text(errors="ignore")
    if "clickable-media-product-modal.js" not in text:
        text = text.replace("</body>", '  <script src="js/clickable-media-product-modal.js?v=1"></script>\n</body>')
    f.write_text(text)

# Correct full address everywhere obvious
full_address = "Musina Mall, C/o Smelter Avenue & Great North Road (N1), Musina. Reza kiosk next to Shoprite."

for f in frontend.glob("*.html"):
    text = f.read_text(errors="ignore")

    text = text.replace("Reza kiosk next to Shoprite", full_address)
    text = text.replace("Reza kioski next to shoprite", full_address)
    text = text.replace("Reza kiosk next to shoprite", full_address)

    # avoid duplicated full address if script ran more than once
    text = text.replace(
...
# Make testimony page better: specific captions
testimony = frontend / "testimony.html"
if testimony.exists():
    text = testimony.read_text(errors="ignore")

    text = text.replace(
        "<span>Musina Mall, C/o Smelter Avenue & Great North Road (N1), Musina. Reza kiosk next to Shoprite.</span>",
        f"<span>{full_address}</span>"
    )
    ...
    text = text.replace(
        "Shared Reza Skincare product experience. Results may vary.",
        "Tap the picture to view it bigger. Customer results may vary."
    )

    testimony.write_text(text)

print("Clickable images/products enabled and address updated.")
PY
